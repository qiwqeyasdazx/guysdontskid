...
local T1UIColor = {
	...
	["Click Effect Color"] = Color3.fromRGB(255, 220, 0),
	["Setting Icon Color"] = Color3.fromRGB(255, 220, 0),
	["Logo Image"] = "rbxassetid://117339791592909",
	["Search Icon Color"] = Color3.fromRGB(255, 220, 0),
	["Search Icon Highlight Color"] = Color3.fromRGB(200, 160, 0),
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(200, 160, 0),   -- amarelo escuro selecionado
	...
	["Toggle Checked Color"] = Color3.fromRGB(255, 240, 100),
	["Toggle Desc Color"] = Color3.fromRGB(255, 255, 255),
	["Button Color"] = Color3.fromRGB(200, 160, 0),          -- amarelo escuro
	["Label Color"] = Color3.fromRGB(255, 220, 0),
	["Dropdown Icon Color"] = Color3.fromRGB(255, 220, 0),
	["Dropdown Selected Color"] = Color3.fromRGB(200, 160, 0),
	["Dropdown Selected Check Color"] = Color3.fromRGB(150, 120, 0),
	["Textbox Highlight Color"] = Color3.fromRGB(255, 220, 0),
	["Box Highlight Color"] = Color3.fromRGB(255, 220, 0),
	["Slider Line Color"] = Color3.fromRGB(255, 220, 0),
	["Slider Highlight Color"] = Color3.fromRGB(200, 160, 0),
...
btnHide.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
btnHide.BackgroundTransparency = 0.4
...
btnHide.ImageTransparency = 0.15
...
function Library:CreateWindow(Setting)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "Abacaxi Hub - Blox Fruit"
	...
	PageControl.BackgroundTransparency = 0.6
	...
	UIPage.Name = "UIPage"
	UIPage.Parent = nil -- NAO adicionar ao MainPage, apenas deixar nil
	...
	function Main_Function:AddTab(PageName)
		...
		PageContainer.BackgroundTransparency = 0.15
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				check.BackgroundColor3 = Color3.fromRGB(255, 220, 0)
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ClickArea_1.BackgroundColor3 = Color3.fromRGB(200, 160, 0)
             ...
             UIGradient_1.Color = ColorSequence.new{
                 ColorSequenceKeypoint.new(0, Color3.fromRGB(200, 160, 0)), 
                 ColorSequenceKeypoint.new(0.4, Color3.fromRGB(220, 180, 0)), 
                 ColorSequenceKeypoint.new(0.6, Color3.fromRGB(255, 220, 0)), 
                 ColorSequenceKeypoint.new(1, Color3.fromRGB(235, 200, 0))
             }
            ...
            function sectionFunction:AddDropdownSection(Setting)
                ...
                Dropdownbg.BackgroundTransparency = 0.5
					...
					if list then
						for _, v in pairs(list:GetChildren()) do
							if not v:IsA("UIListLayout") then
								...
								if btn then
									btn:FindFirstAncestorWhichIsA("ScreenGui") -- ensure loaded
...
task.delay(3.5, function()
	pcall(function()
		Library:Notify({
			Title = "Abacaxi Hub",
			Content = "Script carregado! Bem-vindo, " .. game.Players.LocalPlayer.Name,
			Icon = "star",
			Duration = 5
		});
	end);
...
InfoTab:AddParagraph({
	Title = "Abacaxi Hub",
	Desc = "Version: Abacaxi V2 | Game: Blox Fruits\nMade for the Abacaxi Community\nAll features are free to use."
});
InfoTab:AddParagraph({
...
InfoTab:AddButton({
	...
	Callback = function()
		...
		Library:Notify({Title = "Abacaxi Hub", Content = "Discord link copied to clipboard!\ndiscord.gg/uVGDsyWVm2", Icon = "bell", Duration = 5});
	end
});
InfoTab:AddParagraph({
	Title = " Credits",
	Desc = "Owners: orchidx, humanodev\n\nAbacaxi Hub - Blox Fruit"
});
...
spawn(function()
	while wait(0.5) do
		pcall(function()
			...
			local hasBones = CheckItemCount and CheckItemCount("Bones", 500);
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("AbacaxiV2") then
			makefolder("AbacaxiV2");
		end;
		if not isfolder("AbacaxiV2/Blox Fruits/") then
			makefolder("AbacaxiV2/Blox Fruits/");
		end;
		if not isfile(("AbacaxiV2/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("AbacaxiV2/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("AbacaxiV2/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveSetting = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("AbacaxiV2/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("AbacaxiV2/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
BringEnemy = function()
	...
	local posVec = typeof(PosMon) == "CFrame" and PosMon.Position or hrp.Position;
	local BringRange = _G.BringRange or 235;
	local MaxMob = _G.MaxBringMobs or 3;
	local count = 0;
	...
	for _, enemy in pairs(workspace.Enemies:GetChildren()) do
		...
		-- Teleporta o NPC para perto do player SEM constraints
		-- Constraints no NPC fazem o servidor desincronizar o hitbox
		local fixedPos = Vector3.new(posVec.X, root.Position.Y, posVec.Z);
		...
		-- Remove qualquer constraint antiga que possa desincronizar
...
-- Task 1: SimulationRadius - aplica uma vez e reconecta no respawn
task.spawn(function()
	...
	-- Reaplica a cada 5s para garantir
	while true do
		task.wait(5);
		applySimRad();
	end;
...
-- Task 3: Stabilizer no player (impede queda quando bring ativo)
task.spawn(function()
	while task.wait(0.3) do
		pcall(function()
			...
			if _B then
				...
				if stab then
					-- Garante que a velocidade nao cause bounce (mantem zerado)
					stab.Velocity = Vector3.zero;
				else
					local bv = Instance.new("BodyVelocity");
					bv.Name = "Stabilizer";
					bv.MaxForce = Vector3.new(0, 4e4, 0);
					bv.Velocity = Vector3.zero; -- Zero: cancela queda sem quicar
					bv.Parent = hrp2;
				end;
			else
				local stab = hrp2:FindFirstChild("Stabilizer");
...
task.spawn(function()
	...
	local _lastCPos = C.Position;
	while task.wait() do
		pcall(function()
			if shouldTween then
				if C and C.Parent == workspace then
					local e = plr.Character and plr.Character.PrimaryPart;
					-- So copia pro player se C esta se MOVENDO (tween ativo)
					-- Se C parou (farm parado), nao copia = player pode se mover
					local cMoving = (C.Position - _lastCPos).Magnitude > 0.1;
					_lastCPos = C.Position;
					if cMoving then
...
function AttackModule:AttackNearest()
	...
	-- Coleta todos os inimigos no range
	local head1, parts1 = nil, {};
	for _, Enemy in ipairs(enemyFolder:GetChildren()) do
		local hrp = Enemy:FindFirstChild("HumanoidRootPart");
		local hum = Enemy:FindFirstChild("Humanoid");
		if not hrp or not hum or hum.Health <= 0 then continue; end;
		if (hrp.Position - charPos).Magnitude > 150 then continue; end;
		if not head1 then
...
task.spawn(function()
	while true do
		...
		for _, v in pairs(workspace.Enemies:GetChildren()) do
			...
			EquipWeapon(_G.Settings.Main["Selected Weapon"]);
			AutoHaki();
			-- Vai ate o mob primeiro
			local _initDist = (v.HumanoidRootPart.Position - (plr.Character and plr.Character.HumanoidRootPart and plr.Character.HumanoidRootPart.Position or Vector3.new())).Magnitude;
			if _initDist > 8 then
...
task.spawn(function()
	while true do
		...
		if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 and boss.PrimaryPart then
			local bossTarget = boss.PrimaryPart.CFrame * CFrame.new(0, FARM_HEIGHT, 0);
		...
		if bestMob then
			local mobTarget = bestMob.HumanoidRootPart.CFrame * CFrame.new(0, FARM_HEIGHT, 0);
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Abacaxi Hub", Content = "Item especial encontrado! Auto Chest parado.", Icon = "bell", Duration = 6});
				return;
			end;
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Abacaxi Hub", Content = "Item especial encontrado!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Abacaxi Hub", Content = "Chest Hop: limite atingido, trocando server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Abacaxi Hub", Content = "Item especial encontrado! Chest Bypass parado.", Icon = "bell", Duration = 6});
				return;
			end;
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Abacaxi Hub", Content = "Chest Hop: trocando server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
task.spawn(function()
	while true do
		...
		if foundMob then
			...
			-- Prende o NPC e flutua o player
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			pcall(function()
				if writefile and isfile then
					local path = "AbacaxiV2/Blox Fruits/" .. plr.Name .. ".json";
					if isfile(path) then writefile(path, "{}"); end;
				end;
			end);
			Library:Notify({Title = "Abacaxi Hub", Content = "Settings resetados! Reexecute o script.", Icon = "bell", Duration = 6});
		end);
...
task.spawn(function()
	...
	while task.wait(0.05) do
		...
		local hrpSFA = char:FindFirstChild("HumanoidRootPart")

		pcall(function()
			local parts, head = getPartsInRange(char, 60)
			if #parts == 0 or not head then return end

			if tool:FindFirstChild("LeftClickRemote") then
				local root = char:FindFirstChild("HumanoidRootPart")
				for _, data in ipairs(parts) do
					local eHrp = data[1]:FindFirstChild("HumanoidRootPart")
					if eHrp and root then
						local dir = (eHrp.Position - root.Position).Unit
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Abacaxi Hub", Content = "Estilo nao encontrado: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Abacaxi Hub", Content = "Compra enviada: " .. styleName, Icon = "check", Duration = 4});
end;
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								...
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Abacaxi Hub", Content = "Chegou em: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Abacaxi Hub", Content = "Quest completa!", Icon = "bell", Duration = 6});
						end);
