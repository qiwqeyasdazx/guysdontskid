...
Gradient(Header, Color3.fromRGB(35, 5, 70), Color3.fromRGB(15, 3, 32), 90)
...
TitleLbl.Size               = UDim2.new(0, 230, 0, 24)
TitleLbl.Position           = UDim2.new(0, 46, 0, 4)
TitleLbl.BackgroundTransparency = 1
TitleLbl.Text               = "TRon VOid Hub"
...
TitleGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.2,  Color3.fromRGB(180, 0,   255)),
    ColorSequenceKeypoint.new(0.4,  Color3.fromRGB(255, 140, 255)),
    ColorSequenceKeypoint.new(0.6,  Color3.fromRGB(130, 0,   210)),
    ColorSequenceKeypoint.new(0.8,  Color3.fromRGB(255, 90,  200)),
    ColorSequenceKeypoint.new(1,    Color3.fromRGB(200, 0,   255)),
})
TitleGrad.Rotation = 0

local SubLbl = Instance.new("TextLabel", Header)
SubLbl.Size               = UDim2.new(0, 230, 0, 12)
SubLbl.Position           = UDim2.new(0, 47, 0, 28)
SubLbl.BackgroundTransparency = 1
SubLbl.Text               = "Blox Fruits:by Phahatti"
SubLbl.TextSize           = 9
...
function Comp.StatusParagraph(parent, label, isActive)
    ...
    dot.BackgroundColor3 = isActive and C.green or C.grayDot
    dot.ZIndex           = 7
    Corner(dot, 10)
    ...
    lbl.BackgroundTransparency = 1
    lbl.Text                  = label .. ": " .. statusText
    lbl.TextSize              = 11
    lbl.Font                  = Enum.Font.Gotham
    lbl.TextColor3            = isActive and C.green or C.textDim
    lbl.TextXAlignment        = Enum.TextXAlignment.Left
    lbl.ZIndex                = 7
    ...
    return {
        Frame = frame,
        Set = function(active)
            lbl.Text       = label .. ": " .. (active and "Active" or "Inactive")
            lbl.TextColor3 = active and C.green or C.textDim
            dot.BackgroundColor3 = active and C.green or C.grayDot
        end
    }
end

function Comp.CurrentValueParagraph(parent, label, initialValue)
    local frame = Instance.new("Frame", parent)
    frame.Size             = UDim2.new(1, 0, 0, 34)
    frame.BackgroundColor3 = C.panelLight
    frame.BorderSizePixel  = 0
    ...
    keyLbl.Size                  = UDim2.new(0.55, -11, 1, 0)
    keyLbl.Position              = UDim2.new(0, 11, 0, 0)
...
function Comp.Button(parent, text, callback)
    ...
    Gradient(btn, Color3.fromRGB(110, 0, 190), Color3.fromRGB(55, 0, 110), 90)
...
function Comp.ButtonConfirm(parent, text, callback)
    local frame = Instance.new("Frame", parent)
    frame.Size                  = UDim2.new(1, 0, 0, 32)
    frame.BackgroundTransparency = 1
    frame.ZIndex                = 5
    ...
    local btn = Instance.new("TextButton", frame)
    btn.Size             = UDim2.new(1, 0, 1, 0)
    btn.BackgroundColor3 = Color3.fromRGB(120, 0, 80)
    btn.BorderSizePixel  = 0
    btn.Text             = text
    btn.TextSize         = 12
    btn.Font             = Enum.Font.GothamBold
    ...
    btn.ClipsDescendants = true
    Corner(btn, 8)
    Stroke(btn, Color3.fromRGB(220, 60, 160), 1, 0.4)
    Gradient(btn, Color3.fromRGB(160, 0, 100), Color3.fromRGB(80, 0, 60), 90)

    local confirmRow = Instance.new("Frame", frame)
    confirmRow.Size             = UDim2.new(1, 0, 0, 32)
    confirmRow.Position         = UDim2.new(0, 0, 1, 4)
    confirmRow.BackgroundColor3 = Color3.fromRGB(14, 6, 30)
    confirmRow.BorderSizePixel  = 0
    confirmRow.ZIndex           = 10
    confirmRow.Visible          = false
    Corner(confirmRow, 8)
    Stroke(confirmRow, C.border, 1, 0.4)

    local confirmLbl = Instance.new("TextLabel", confirmRow)
    confirmLbl.Size                  = UDim2.new(0.42, 0, 1, 0)
    confirmLbl.Position              = UDim2.new(0, 8, 0, 0)
    confirmLbl.BackgroundTransparency = 1
    confirmLbl.Text                  = "Confirm?"
    confirmLbl.TextSize              = 11
    confirmLbl.Font                  = Enum.Font.Gotham
    confirmLbl.TextColor3            = C.text
    confirmLbl.TextXAlignment        = Enum.TextXAlignment.Left
    confirmLbl.ZIndex                = 11

    local yesBtn = Instance.new("TextButton", confirmRow)
    yesBtn.Size             = UDim2.new(0, 48, 0, 22)
    yesBtn.Position         = UDim2.new(1, -106, 0.5, -11)
    yesBtn.BackgroundColor3 = C.green
    ...
    local noBtn = Instance.new("TextButton", confirmRow)
    noBtn.Size             = UDim2.new(0, 48, 0, 22)
    noBtn.Position         = UDim2.new(1, -54, 0.5, -11)
    ...
    btn.MouseEnter:Connect(function()
        Tween(btn, 0.18, nil, nil, {BackgroundColor3 = Color3.fromRGB(160, 0, 110)})
    end)
    btn.MouseLeave:Connect(function()
        Tween(btn, 0.18, nil, nil, {BackgroundColor3 = Color3.fromRGB(120, 0, 80)})
    end)
...
function Comp.Dropdown(parent, label, opts, callback)
    ...
    for _, opt in ipairs(opts) do
        ...
        ob.MouseEnter:Connect(function()
            Tween(ob, 0.14, nil, nil, {BackgroundColor3 = C.purple2, BackgroundTransparency = 0})
        end)
        ob.MouseLeave:Connect(function()
            Tween(ob, 0.14, nil, nil, {BackgroundColor3 = C.purple3, BackgroundTransparency = 0.5})
        end)
...
function Comp.DropdownMulti(parent, label, opts, callback)
    local selected   = {}
    local expanded   = false
    local checkMarks = {}
    local totalH     = #opts * 27 + 10
    ...
    header.ClipsDescendants = true
    Corner(header, 8)
    Stroke(header, Color3.fromRGB(100, 20, 180), 1, 0.4)

    local hLbl = Instance.new("TextLabel", header)
    hLbl.Size                  = UDim2.new(1, -64, 1, 0)
    hLbl.Position              = UDim2.new(0, 10, 0, 0)
    hLbl.BackgroundTransparency = 1
    hLbl.Text                  = label .. ":  (none)"
    hLbl.TextSize              = 12
    hLbl.Font                  = Enum.Font.Gotham
    hLbl.TextColor3            = C.text
    hLbl.TextXAlignment        = Enum.TextXAlignment.Left
    ...
    local selectAllBtn = Instance.new("TextButton", container)
    selectAllBtn.Size             = UDim2.new(0, 30, 0, 18)
    selectAllBtn.Position         = UDim2.new(1, -58, 0, 7)
    selectAllBtn.BackgroundColor3 = C.purple2
    selectAllBtn.BorderSizePixel  = 0
    selectAllBtn.Text             = "All"
    selectAllBtn.TextSize         = 9
    selectAllBtn.Font             = Enum.Font.GothamBold
    selectAllBtn.TextColor3       = C.white
    selectAllBtn.ZIndex           = 9
    Corner(selectAllBtn, 4)
    ...
    dropList.ClipsDescendants = true
    Corner(dropList, 8)
    Stroke(dropList, Color3.fromRGB(100, 20, 180), 1, 0.4)
    ...
    local function UpdateHeader()
        if #selected == 0 then
            hLbl.Text = label .. ":  (none)"
        elseif #selected == #opts then
            hLbl.Text = label .. ":  (all)"
        else
            hLbl.Text = label .. ":  " .. table.concat(selected, ", ")
        end
    end
    ...
    for _, opt in ipairs(opts) do
        ...
        local ckBox = Instance.new("Frame", ob)
        ckBox.Size             = UDim2.new(0, 14, 0, 14)
        ckBox.Position         = UDim2.new(0, 6, 0.5, -7)
        ckBox.BackgroundColor3 = C.purple3
        ckBox.ZIndex           = 22
        Corner(ckBox, 4)
        Stroke(ckBox, C.purpleLight, 1, 0.3)

        local ckMark = Instance.new("TextLabel", ckBox)
        ckMark.Size                  = UDim2.new(1, 0, 1, 0)
        ckMark.BackgroundTransparency = 1
        ckMark.Text                  = "✓"
        ckMark.TextSize              = 10
        ckMark.Font                  = Enum.Font.GothamBold
        ckMark.TextColor3            = C.purpleLight
        ckMark.ZIndex                = 23
        ckMark.Visible               = false

        local optLbl = Instance.new("TextLabel", ob)
        ...
        ob.MouseButton1Click:Connect(function()
            ...
            if idx then
                ...
                ckMark.Visible         = true
                ckBox.BackgroundColor3 = C.purple2
            end
            UpdateHeader()
            if callback then task.spawn(callback, selected) end
        end)
    ...
    selectAllBtn.MouseButton1Click:Connect(function()
        if #selected == #opts then
            selected = {}
            for _, opt in ipairs(opts) do
                checkMarks[opt].Visible = false
            end
        else
            selected = {}
            for _, opt in ipairs(opts) do
                table.insert(selected, opt)
                checkMarks[opt].Visible = true
            end
        end
        UpdateHeader()
        if callback then task.spawn(callback, selected) end
    ...
    return {
        ...
        GetAll = function() return opts end
    }
end
...
function Comp.Slider(parent, text, minV, maxV, defV, callback)
    ...
    fill.ZIndex           = 7
    Corner(fill, 5)
    Gradient(fill, Color3.fromRGB(100, 0, 180), Color3.fromRGB(200, 70, 255), 0)
...
function Comp.DiscordCard(parent, guildId, inviteCode)
    ...
    joinBtn.MouseButton1Click:Connect(function()
        ...
        pcall(function()
            setclipboard("https://discord.gg/" .. (inviteCode or "eeTwpuGaVa"))
        end)
    ...
    task.spawn(function()
        ...
        if ok and res then
            ...
            totLbl.Text  = tostring(total  or "?") .. " Members"
        else
            onLbl.Text  = "Join the discord!"
            totLbl.Text = "discord.gg/eeTwpuGaVa"
        end
    end)
...
local mTab = tabFrames["Tab | Main Testing"]

Comp.Section(mTab, "DISCORD SERVER")
Comp.DiscordCard(mTab, DISCORD_ID, "eeTwpuGaVa")

Comp.Section(mTab, "TOGGLES")
Comp.Toggle(mTab, "Auto Farm Active", false, function(v)
    print("[TRon VOid] Auto Farm:", v)
end)
Comp.Toggle(mTab, "Auto Quest", true, function(v)
    print("[TRon VOid] Auto Quest:", v)
end)
Comp.Toggle(mTab, "God Mode (Visual)", false, function(v)
    print("[TRon VOid] God Mode:", v)
end)
Comp.Toggle(mTab, "No Clip", false, function(v)
    print("[TRon VOid] No Clip:", v)
end)

Comp.Section(mTab, "BUTTONS")
Comp.Button(mTab, "🔗  Connect to Server", function()
    print("[TRon VOid] Connecting...")
end)
Comp.Button(mTab, "🔄  Rejoin Server", function()
    print("[TRon VOid] Rejoin...")
end)
Comp.ButtonConfirm(mTab, "⚠️  Reset Character", function()
    print("[TRon VOid] Confirmed: Reset Character")
end)
Comp.ButtonConfirm(mTab, "🗑️  Clear Data", function()
    print("[TRon VOid] Confirmed: Clear Data")
end)

Comp.Section(mTab, "DROPDOWNS")
Comp.Dropdown(mTab, "Region", {"Auto", "US East", "US West", "EU", "Asia", "BR"}, function(v)
    print("[TRon VOid] Region:", v)
end)
Comp.Dropdown(mTab, "Game Mode", {"Normal", "Hard", "Extreme", "Chaos"}, function(v)
    print("[TRon VOid] Mode:", v)
end)
Comp.DropdownMulti(mTab, "Active Skills", {"Sword", "Gun", "Fruit", "Melee", "Dash"}, function(v)
    print("[TRon VOid] Skills:", table.concat(v, ", "))
end)
Comp.DropdownMulti(mTab, "Farm Targets", {"Boss", "NPC", "Chest", "Quest NPC"}, function(v)
    print("[TRon VOid] Targets:", table.concat(v, ", "))
end)

Comp.Section(mTab, "SLIDERS")
Comp.Slider(mTab, "Walk Speed", 0, 500, 16, function(v)
    pcall(function()
        LocalPlayer.Character.Humanoid.WalkSpeed = v
    end)
end)
Comp.Slider(mTab, "Jump Power", 0, 500, 50, function(v)
    pcall(function()
        LocalPlayer.Character.Humanoid.JumpPower = v
    end)
end)
Comp.Slider(mTab, "Field of View (FOV)", 60, 120, 70, function(v)
    print("[TRon VOid] FOV:", v)
end)
Comp.Slider(mTab, "Character Transparency", 0, 100, 0, function(v)
    print("[TRon VOid] Transparency:", v)
end)

Comp.Section(mTab, "PARAGRAPHS")
Comp.Label(mTab, "Standard paragraph label — use for descriptions and notes.")
Comp.StatusParagraph(mTab, "Auto Farm", false)
Comp.StatusParagraph(mTab, "Server Status", true)
Comp.CurrentValueParagraph(mTab, "Current Level", "117")
Comp.CurrentValueParagraph(mTab, "Current Quest", "None")
...
task.delay(0.1, function()
    SwitchTab("Tab | Main Testing")
end)

print("[TRon VOid Hub] UI Loaded Successfully! ✓")
